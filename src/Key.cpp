...
bool Key::Load(std::string filename)
{
...
void Key::Render(Player &player, vec3 pos)
{
    if(taken != 1)
    {
	...
	glPushMatrix();
		...
		glTranslatef(pos.x, pos.y + 0.2f, pos.z);
...
void Key::Update(Player &player)
{
    ...
    rotation = 0.f;
     ...
     if(sqrt((player.pos.x - pos.x)*(player.pos.x - pos.x) + (player.pos.z - pos.z)*(player.pos.z - pos.z) + (player.pos.y - pos.y)*(player.pos.y - pos.y)) <= 0.4 )
     {
         //printf("znikaj!\n");
         taken = 1;
         player.keys_counter ++;
	...
	// {
	// 	for (int j = i + 1; j < KeyObjects.size(); j++)
	// 	{
	// 		KeyObject* obj1 = KeyObjects[i];
	// 		KeyObject* obj2 = KeyObjects[j];
