...
bool Heart::Load(std::string filename)
{
...
void Heart::Render(Player &player, vec3 boxposition,bool taken)
{
    if(taken != 1)
    {
    ...
    Update(player, boxposition);
	glDisable(GL_TEXTURE_2D);
    }
}

void Heart::Update(Player &player, vec3 pos)
{
    if(rotation <= 360.f)
    rotation += 1.f;
    else
	...
	// for (int i = 0; i < HeartObjects.size(); i++)
	// {
	// 	for (int j = i + 1; j < HeartObjects.size(); j++)
	// 	{
	// 		HeartObject* obj1 = HeartObjects[i];
	// 		HeartObject* obj2 = HeartObjects[j];
