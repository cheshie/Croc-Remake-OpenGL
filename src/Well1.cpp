...
bool Well1::Load(std::string filename)
{
...
void Well1::Render(Player &player)
{
    //if(taken != 1)
    //{
...
void Well1::Update(Player &player)
{
    // if(rotation <= 360.f)
    // rotation += 1.f;
    // else
    ...
    if(sqrt((player.pos.x - pos.x)*(player.pos.x - pos.x) + (player.pos.z - pos.z)*(player.pos.z - pos.z)) <= 0.65f)
    {

        if(player.pos.y - (pos.y ) <= 1.3 && ((pos.y ) < player.pos.y) && sqrt((player.pos.x - pos.x)*(player.pos.x - pos.x) + (player.pos.z - pos.z)*(player.pos.z - pos.z)) > 0.4f)
        {



			player.stopwell1 = 1;		
			player.standat = pos.y + 0.3f;
			player.maxrelativeheight = pos.y + 0.3f;

			if( fabs(player.pos.y - pos.y) <= 0.3f)
			player.jumping = 0;
		...
		if (((pos.y) > player.pos.y))
		{
			...
			if(player.stop == 0 && sqrt((player.pos.x - pos.x)*(player.pos.x - pos.x) + (player.pos.z - pos.z)*(player.pos.z - pos.z)) > 0.4f)
			{
